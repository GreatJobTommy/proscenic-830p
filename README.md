...
**Preferred:** Settings → Devices → Add integration → **Proscenic 830P**. Choose *ProscenicHome email code*. Enter the app email (Germany = country code 49). A 6-digit code is emailed; the app password is not used. The flow fetches `local_key` once and then talks LAN only.
